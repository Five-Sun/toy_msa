...
public class UserController {
    ...
    @GetMapping("/check/{productId}/stock")
    public ResponseEntity<String> checkCatalogStock(@PathVariable String productId) {
        try {
            int stock = userService.getCatalogStock(productId);
            String message;
            if (stock >= 0) {
                message = "상품 재고:" + stock;
            } else {
